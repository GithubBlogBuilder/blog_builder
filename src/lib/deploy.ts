...
abstract class Deploy {
     ...
     */
    static async createRepoFromTemplate(username: string, repo: string): Promise<void> {
        try {
            await this.octokit.request("POST /repos/{template_owner}/{template_repo}/generate", {
                template_owner: "GithubBlogBuilder",
                template_repo: "blog_builder_default_template",
                name: repo,
                owner: username,
                description: 'A blog website generate with "GithubBlogBuilder/blog_builder_default_template".',
                headers: headers,
            });
        } catch (e) {
            console.error(e);
    ...
    static async getWorkflowRunIds(username: string, repo: string): Promise<number[]> {
        try {
    ...
    static async getWorkflowRunStatus(username: string, repo: string, runId: number): Promise<WorkflowStates | undefined> {
        try {
    ...
    static async reRunWorkflow(username: string, repo: string, runId: number): Promise<void> {
        try {
